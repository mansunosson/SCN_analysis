...
%% Data import %%
vData = VideoReader('cry1_exp2.avi');
SCN = read(vData);
zdata = im2double(SCN(:,:,:,:));
cry1 = reshape(zdata(:,:,3,1:233), size(zdata,1), size(zdata,2), size(zdata,4)); %Blue light
T  = size(cry1,3);
...
toc  
save('OUT CRY1 Exp2.mat','tauest', 'thetaest', 'REest', 'thetameanest', 'psiest', 'a', 'b', 'B', 'dt', 'samp','LIK0','tauprop','REprop','psiprop','thetameanprop', 'tauaccept', 'REaccept', 'thetameanaccept', '-v7.3');
